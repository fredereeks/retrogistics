...
export default function BlogCard({id, image, title, text, wide }) {
    const link = title.toLowerCase().replaceAll("'","").replaceAll(" ", "-").replaceAll("!","");
    return (
        <aside className={`py-3 flex gap-2 relative ${wide ? 'w-full' : 'max-w-4xl mx-auto'}`}>
            <Link state={{image,title,text,}} to={`/blogs/${id}/${link}`} className="absolute top-0 left-0 w-full h-full opacity-0"></Link>
            <img src={image} alt={title} className={`${wide ? 'w-48 sm:w-80' : 'w-40 sm:w-52'} object-cover`} />
